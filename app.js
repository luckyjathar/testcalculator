...
function renderRows(pIdx) {
    ...
    document.getElementById(`body_${pIdx}`).innerHTML = prod.calculatedData.map(d => {
        ...
        return `<tr id="row_${pIdx}_${d.dIdx}" class="${rowClass}">
            ...
            <td><input id="l_${pIdx}_${d.dIdx}" type="number" value="${Math.floor(d.loan)}" class="step-inp" onchange="manual(${pIdx},${d.dIdx})" onblur="manual(${pIdx},${d.dIdx})" style="padding: 6px; border: 1px solid #ccc; border-radius: 4px; text-align: center; width: 80px; font-weight: bold; color: var(--primary);"></td>
