...
function saveEmiDraft() {
    ...
    updateDraftBadgeCount(); showToast("✅ Draft Successfully Saved! (PC रिस्टार्ट केल्यावर सुद्धा हा ड्राफ्ट राहील)", "success");
...
function generateFlyer() {
    ...
    if(!tempFgBitly) { showToast("⚠️ Is Dealer ki Bitly Link master data mein nahi mili! Kripya apni Excel file check karein.", "error"); return; }
...
function copyFlyerLink() { let copyText = document.getElementById('fgGeneratedLinkText'); copyText.select(); copyText.setSelectionRange(0, 99999); document.execCommand("copy"); showToast("✅ Link Copied! Aap yeh link Customer ko WhatsApp par bhej sakte hain.", "success"); }
function shareOnWhatsAppStatus() { let generatedLink = document.getElementById('fgGeneratedLinkText').value; if (!generatedLink) { showToast("⚠️ Pehle link generate karein!", "error"); return; } let statusMessage = "🔥 *Festival Special Offers!* 🔥\n\nNaya Mobile, Laptop ya TV lene ka soch rahe ho? Zero percent interest (0% EMI) par kharidi karein!\n\nAaj hi apni eligibility (Limit) check karein aur attractive offers payein. Neeche di gayi link par click karein 👇\n\n" + generatedLink; let encodedMessage = encodeURIComponent(statusMessage); window.open(`https://wa.me/?text=${encodedMessage}`, '_blank'); }

function openDictionaryModal() { 
    if(!db_records || db_records.length === 0) { showToast("⚠️ Schemes Dictionary background se load ho rahi hai, kripya 2 second wait karein...", "warning"); return; } 
...
function renderTableModel() {
    ...
    validSchemes = validSchemes.filter(s => s.fixedEmi > 0 || s.calcLTV <= ltvLimit);
...
async function addCustomerToQueue() {
    let name = document.getElementById('cqName').value || `Cust ${customerQueue.length + 1}`; let mobile = document.getElementById('cqMobile').value || ""; let limit = parseFloat(document.getElementById('cqLimit').value); let ltv = parseFloat(document.getElementById('cqLtv').value) || 100; let type = document.getElementById('cqType').value; let cap = parseFloat(document.getElementById('cqCap').value) || "";
    if(!limit || limit <= 0) { showToast("⚠️ Customer add karne ke liye pehle ek valid NBFC LIMIT enter karein!", "error"); return; }
    let now = new Date(); let ts = now.toLocaleDateString('en-GB', {day:'2-digit', month:'short'}) + ' ' + now.toLocaleTimeString('en-US', {hour:'2-digit', minute:'2-digit'});
...
function recalcModel(pIdx) {
    ...
    prod.calculatedData = prod.schemes.map((s, dIdx) => {
        ...
        if (isFixed) {
            ...
            if(!isManuallyOverridden && emi > parseFloat(inp.cap) && parseFloat(inp.cap) > 0) { loan = (parseFloat(inp.cap) - (insTotal/inst)) / ( (1/s.tenure) + roiRate ); roiInEmi = loan * roiRate; emi = (loan / s.tenure) + (insTotal / inst) + roiInEmi; } let roiInDp = loan * roiRateDP; dpExact = effectivePrice - loan + ((loan / s.tenure) * s.advEmi) + (loan * dbdRate) + dynamicPf + totalFees + roiInDp;
        }
...
function fallbackCopy(text, successCb) {
    ...
    try { let success = document.execCommand('copy'); if (success) { successCb(); } else { showCustomAlert("Auto-copy block zala ahe. Kripya khali dileli mahiti manually copy kara:", text); } } catch (err) { showCustomAlert("Auto-copy fail zala. Kripya khali dileli mahiti manually copy kara:", text); } document.body.removeChild(ta);
}
...
function doGenerateCustomerImage() {
    ...
    productsToRender.forEach((prod) => {
        ...
        let badgeDetails = {
            "DP": { text: "🟢 Save Cash Today", color: "#10b981" },
            "EMI": { text: "🔵 Pocket Friendly EMI", color: "#3b82f6" },
            "POP": { text: "🟡 Most Loved", color: "#f59e0b" },
            "BAL": { text: "🟣 Super Value", color: "#8b5cf6" }
        };
    ...
    html2canvas(quoteDiv, {scale: 2}).then(canvas => { 
        ...
        if(requestWhatsAppDispatch) {
            ...
            if (isMobile) {
                let a = document.createElement("a"); a.href = imgDataUrl; let safeName = c?.name !== "-" ? c.name.replace(/\s+/g, '_') : "Customer"; a.download = `Quotation_${safeName}.png`; document.body.appendChild(a); a.click(); document.body.removeChild(a); showToast("✅ Image download ho gayi hai! Ab WhatsApp khulega.", "success");
                ...
                canvas.toBlob(blob => {
                    try { navigator.clipboard.write([ new ClipboardItem({ "image/png": blob }) ]).then(() => { document.getElementById('clipboardStatusAlert').style.display = 'block'; setTimeout(() => { let mobile = c?.mobile || ""; if(mobile && mobile.length >= 10) window.open(`https://wa.me/91${mobile}`, '_blank'); }, 500); }).catch(err => { showCustomAlert("📋 Click / Long Press karke Copy Image karein."); let mobile = c?.mobile || ""; if(mobile && mobile.length >= 10) window.open(`https://wa.me/91${mobile}`, '_blank'); }); } 
