...
/* === MANUAL EXCEL FILE UPLOAD HANDLER === */
let excelInputEl = document.getElementById('excelUploadInput');
...
if (excelInputEl) {
    excelInputEl.addEventListener('change', function(e) {
        ...
        showToast("⏳ Excel फाईल रीड करत आहे, कृपया प्रतीक्षा करा...", "warning");
        ...
        reader.onload = async function(event) {
            try {
                ...
                parsedSheet2Data = wb.SheetNames.length > 1 ? 
                    XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[1]], { raw: false, defval: "" })
                    .map(r => mapData(r, SPECIAL_MODEL))
                    .filter(x => x && x.model && x.model.trim() !== "") : [];
                ...
                if (uniqueDB.length > 0) {
                    ...
                    showToast("✅ अपलोड केलेली Excel फाईल यशस्वीरित्या सिंक झाली!", "success");
                } else {
                    showToast("⚠️ फाईलमध्ये कोणताही योग्य डेटा सापडला नाही.", "error");
                }
            } catch (error) {
                console.error("Excel Parsing Error: ", error);
                showToast("⚠️ Excel फाईल प्रोसेस करताना एरर आला. फाईल फॉरमॅट तपासा.", "error");
            }
        };
